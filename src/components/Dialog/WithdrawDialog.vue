...
export default {
  ...
  computed: {
    ...
    coinBalance () {
      return this.depositsLists[this.depositsItemIndex].pTokenBalance * this.depositsLists[this.depositsItemIndex].market.exchangeRate + ''
    },
    cardLists () {
      let lists = [
        {
  ...
  watch: {
    ...
    inputValue: {
      handler () {
        if (parseFloat(this.inputValue) > parseFloat(this.coinBalance)) {
  ...
  methods: {
    ...
    handleWithdraw () {
      ...
      this.$writeLoad(pTokenContractAddress, this.getPTokenAbi, 'redeemUnderlying', price).then(res => {
        if (res) {
