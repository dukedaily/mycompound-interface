...
export function getContract (contractAddress, abi, funcName, ...params) {
  ...
  return new Promise((resolve, reject) => {
    contract[funcName](...params).then(response => {
      resolve(response)
    }, err => { // 合约调用错误
      console.log(err)
      reject(605)
...
export function getWriteContract (contractAddress, abi, funcName, ...params) {
  ...
  return new Promise((resolve, reject) => {
    contractWithSigner[funcName](...params).then(response => {
      resolve(response)
    }, err => {
      reject(err)
    })
 ...
 * @returns promise
 */
export function getWriteContractLoad (contractAddress, abi, funcName, ...params) {
  const provider = new ethers.providers.Web3Provider(window.ethereum)
  const contract = new ethers.Contract(contractAddress, abi, provider)
  const contractWithSigner = contract.connect(provider.getSigner())
  return new Promise((resolve, reject) => {
